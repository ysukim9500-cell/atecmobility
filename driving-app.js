...
(function () {
  ...
  function loadAll() {
    ...
    Promise.all([
      ...
      fetchAll('/rest/v1/evidences?select=id,username,vehicle_plate,date_millis,category,amount,memo,photo_path&order=id.asc'),
  ...
  function viewEvid() {
    ...
    var locked = cycleApproved(myName());
    var h = head(scopeTitle('증빙'), esc(cycleName(CYC.y, CYC.m)) +
    ...
    h += sect('내역', rows.length + '건', upBtn,
      ...
      rows.map(function (e) {
        return '<tr><td><span class="lead">' + md(e.date_millis) + '</span></td>' +
          ...
          (!locked && e.username === myName()
            ? ' <button class="btn sm" data-evdel="' + esc(e.id) + '">지우기</button>' : '') +
          '</td></tr>';
      }).join('') + '</tbody></table></div></div>');
    return h;
  }
