...
(function (root) {
  ...
  var ACTION_KB = {
    BMS_DOWN:      { name:'BMS 통신 단절',   steps:['외장모뎀 전원 LED(ST1/ST2/Pow/LTE) 확인','모뎀 USIM 세척·재삽입·커넥터 접촉','외장 LTE 모뎀 교체','LTE 케이블 교체','통합단말기(BMS B/D) 교체'], parts:['외장 LTE 모뎀','LTE 케이블','통합단말기(BMS B/D)'], note:'외장모뎀 전원 전체 꺼짐이면 통합단말기 BMS B/D 불량 · B600/B700/B710 외장모뎀 혼용 금지 · 교체 전 거래백업' },
    BMS_WARN:      { name:'BMS 통신 이상',   steps:['모뎀 USIM 세척·재삽입','모뎀 LED·커넥터 점검'], parts:['외장 LTE 모뎀'] },
    MQTT_PATH:     { name:'BMS↔TOPIS 통신경로 이상', steps:['외장모뎀 LED·신호감도 확인','모뎀 USIM 세척·재삽입','모뎀 안테나 점검','외장 LTE 모뎀 교체'], parts:['외장 LTE 모뎀','모뎀 안테나'] },
    SERVER_CONN:   { name:'서버연결 불량',   steps:['표출기 LTE 감도·외장모뎀 LED 점검','수집/단말 센터 IP 설정 확인(재설치 메뉴)','지속 시 통합단말기 교체 검토'], parts:['외장 LTE 모뎀','통합단말기'] },
    SERVER_DEAD:   { name:'서버연결 전면 실패', steps:['거래내역 백업 선행(유실 방지)','외장모뎀·네트워크 활성/LED 점검','통합단말기 교체','안 되면 표출단말기 교체'], parts:['외장 LTE 모뎀','통합단말기','표출단말기'], note:'미전송 유실 위험 — 백업 우선 · 승하차 번쩍거림 동반 시 통합+승하차 동시 교체(순서 준수)' },
    TRANS_BACKLOG: { name:'거래 미전송 누적', steps:['거래내역 백업 선행','표출기 LTE 감도·외장모뎀 LED 확인','외장모뎀 안테나 점검·교체','지속 시 통합단말기 교체'], parts:['외장 LTE 모뎀','모뎀 안테나','통합단말기'], note:'거래내역 전송불가 지속 시 수리센터 입고(운전자단말 백업 선행)' },
    ...
    WATCHDOG:      { name:'워치독 과다리셋', steps:['반복 리셋 유발 모듈 확인(BMS 통신 이상 Reset 동반 여부)','통신 복구(외장모뎀 점검)','지속 시 통합단말기 교체'], parts:['외장 LTE 모뎀','통합단말기'] },
    REBOOT_LOOP:   { name:'연속재부팅',     steps:['전원 케이블 체결·접촉 확인','차량 Fuse 확인·교체','전원 케이블 임시교체 TEST','연속 재부팅이면 부팅불량 — 통합단말기(본체) 우선 점검·교체'], parts:['전원 케이블','차량 퓨즈','통합단말기(전원부/본체)'], note:'5분 내 연속 재부팅은 부팅 중 멈춤(부팅불량)일 수 있어 본체가 유력 후보' },
    POWER_ABN:     { name:'비정상 전원차단', steps:['차량 Fuse 확인·교체','전원 케이블 재연결/임시교체 TEST','지속 시 통합단말기 전원부/본체 점검·교체'], parts:['차량 퓨즈','전원 케이블','통합단말기(전원부)'] },
    POWER_TREND:   { name:'전원/재부팅 열화', steps:['차량 Fuse·전원 케이블 체결 점검','전원 케이블 임시교체 TEST','반복 지속 시 통합단말기 전원부/본체 점검·교체'], parts:['차량 퓨즈','전원 케이블','통합단말기(전원부)'] },
    GPS_WARN:      { name:'GPS 수신 불량',   steps:['GPS 안테나 커넥터 체결·하늘 시야 확보 확인','안테나 케이블 접촉 점검(위치 변경)','GPS 안테나 교체','지속 시 통합단말기 교체'], parts:['GPS 안테나','통합단말기'] },
    GPS_ANT:       { name:'GPS 안테나 이상(편측)', steps:['해당(앞/뒤) 안테나 커넥터 재연결','안테나 위치 변경(시야 확보)','안테나 교체','지속 시 통합단말기 교체'], parts:['GPS 안테나','통합단말기'] },
    DATA_LOSS:     { name:'데이터 유실(누락)', steps:['거래내역 즉시 백업(유실 예방)','저장장치·전송 경로 점검','재발 시 해당 단말기 점검'], parts:['해당 단말기'] },
    ...
    CARD_READER:   { name:'카드리더 오류 과다', steps:['카드 TAG부·RF 안테나부 청소','SAM(PSAM) 세척·소켓 접촉 확인','소프트 재부팅','재발 시 해당 단말기 교체(1:1 권장)'], parts:['PSAM','RF 모듈','승하차 단말기'] },
    SEUNGHA_BOOT:  { name:'승하차 부팅/전원 이상', steps:['승하차 전원 커넥터·케이블 체결 점검','통합단말기 포트 Val→sp 이동해 격리','펌웨어 배포 여부 확인','증상 지속 시 승하차 단말기 교체'], parts:['승하차 단말기','전원 케이블','통합단말기(포트)'], note:'신규 신호 — 현장 확인으로 검증 중 · 전체 번쩍거림이면 통합+승하차 동시 교체' },
    ...
    FAN_FAULT:     { name:'냉각팬 이상', steps:['팬 동작·이물질 확인','팬 교체','방치 시 과열로 본체 손상 주의'], parts:['냉각팬','통합단말기'] }
  };
  // 이벤트 그룹별 조치 (EVT_GROUP 발화 시 그룹에 맞는 가이드로 대체)
  var EVT_GROUP_ACTIONS = {
    '승하차통신': { steps:['해당 단말기 커넥터 체결 확인','통신 케이블 임시 가설 TEST','통합단말기 포트 Val→sp 이동 점검','재발 시 승하차 단말기 → 통합단말기 순 교체'], parts:['통신 케이블','승하차 단말기','통합단말기'] },
    '표출기통신': { steps:['표출기↔통합 케이블 접점·삽입 점검','표출기 재부팅','지속 시 표출단말기 교체'], parts:['표출기 케이블','표출단말기'] },
    '카드SAM':   { steps:['PSAM 세척·소켓 접촉 확인','재인식 후에도 지속 시 해당 단말기 교체(1:1 권장)'], parts:['PSAM','승하차 단말기'] },
    'GW통신':    { steps:['해당 보드 커넥터·케이블 점검','지속 시 통합단말기 교체 검토'], parts:['케이블','통합단말기'] },
    '전원HW':    { steps:['차량 Fuse 확인','전원 케이블 재연결','지속 시 통합단말기 점검'], parts:['차량 퓨즈','전원 케이블','통합단말기'] },
    '펌웨어':    { steps:['F/W·OS 재설치(USB) 또는 초기화 후 재등록','복구 실패 시 본체 교체'], parts:['설치 USB','CPU B/D'] },
    '운행':      { steps:['승하차 통신·타코메타 설정 확인','표출기 점검','재발 시 해당 승하차/표출 단말기 교체'], parts:['표출단말기','승하차 단말기','타코메타'] },
    '모뎀BMS통신': { steps:['외장모뎀 전원 LED(ST1/ST2/Pow/LTE) 확인','USIM·커넥터 접촉 점검','외장 LTE 모뎀 교체 → LTE 케이블 → 통합단말기 순'], parts:['외장 LTE 모뎀','LTE 케이블','통합단말기'] },
    '센터통신':   { steps:['외장모뎀 신호·안테나 점검','수집/단말 센터 IP 설정 확인(재설치 메뉴)','지속 시 통합단말기 교체'], parts:['외장 LTE 모뎀','안테나','통합단말기'] },
    'GPS위치':    { steps:['GPS 안테나 체결·하늘 시야 확보 확인','안테나 케이블 접촉 점검 → 안테나 교체','타코/도어 센서 동반 이상 시 함께 점검','지속 시 통합단말기 교체'], parts:['GPS 안테나','타코/도어 센서','통합단말기'] },
    '기타':      { steps:['해당 부위 세부 이벤트 확인','연결 케이블·커넥터 점검'], parts:['케이블'] }
  };
  ...
  function plainOf(f){
    ...
    switch(f.code){
      ...
      case 'REBOOT_LOOP': return '단말기가 켜진 지 5분도 안 돼 다시 꺼졌다 켜지기를 '+(_num(ev,/재시작 (\d+)회/)||'여러')+'번 반복했습니다. 전원 케이블·퓨즈뿐 아니라 통합단말기 본체(부팅불량)도 유력 후보입니다.';
      case 'POWER_ABN': return '전원이 정상 종료 절차 없이 뚝 끊긴 기록이 '+(_num(ev,/재시작 (\d+)회/)||'여러')+'번 있습니다. 차량 퓨즈·전원 케이블을 먼저 보고, 지속되면 통합단말기 전원부도 점검하세요.';
      case 'POWER_TREND': return '이 단말기는 예전부터 재부팅이 잦았습니다. 전원 계통(퓨즈·케이블)과 함께 통합단말기 전원부/본체 열화도 의심하세요.';
  ...
  var _CAUSE={
    ...
    REBOOT_LOOP:'전원 계통·통합단말기 본체 (연속재부팅=본체 유력)', POWER_ABN:'전원 계통 (퓨즈·케이블·통합단말기 전원부)', POWER_TREND:'전원 계통 (퓨즈·케이블·통합단말기 전원부)',
